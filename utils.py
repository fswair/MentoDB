...
class Column:
    def __init__(self, arg: str, is_primary: bool = False):
    ...
    def alphanum(self, arg: str):
        data = [
            letter for letter in arg if letter.isalnum()
        ]
        return "".join(data)
...
class Mento:
    ...
    def create(self, table: str = None, model: BaseModel = DefaultModel, exists_check: bool = True):
        ...
        for param in parameters:
            column = Column(str(param))
            if not column.has_unique_check:
                columns.append(column.arg)
        create_query = ", ".join(columns)
        if exists_check:
            self.connection.execute(f"CREATE TABLE IF NOT EXISTS {table} ({create_query})")
        else:
            try:
    ...
    def create_many(self, datas: dict = dict(user=DefaultModel), exists_check: bool = True):
    ...
    def insert(self, table: str, data: dict, check_model: BaseModel = None):
        ...
        if check_model:
            ...
            if conditions:
                cursor = self.connection.execute(f"SELECT * FROM {table} where {where_query}")
                fetch = Fetch(cursor)
    ...
    def update(self, table: str = None, data: dict = None, where: dict = None, update_all: bool = False):
        ...
        queries = list()
        for k,v in data.items():
            if not type(v) == int: v = f"'{v}'" 
            queries.append(f"{k}={v}")
        update_query = ", ".join(queries)
        if update_all:
            self.connection.execute(f"UPDATE {table} SET {update_query}")
        else:
            self.connection.execute(f"UPDATE {table} SET {update_query} where {where_statement} ")

        # return self.select(table, where={f"{key}":value}, select_all=update_all)
                            ...
                            if iterable(regexp[column]):
                                for regex in regexp[column]:
                                    if column not in fetch.columns:
                                        raise BaseException(f"Current table has no column named `{column}`.")
    ...
    def delete(self, table: str, where: dict = dict(), delete_all: bool = False):
        ...
        if delete_all:
            ...
            if where:
                ...
                raise BaseException("Please add where statement or set delete_all as true to delete all rows.")
